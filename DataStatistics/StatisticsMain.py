        ...
        if FileNum > 10:  # 判断分片文件是否大于10，若大于每次创建10个线程，小与10个创建与文件数相同线程
            while f < (FileNum / 10):  # 每次创建10个进程    创建文件数/10 次
                ...
                while i < 10:  # 每次读取10个文件名
                    ...
                    i += 1
                MyMultithreadingStatistics.create_Thread(ThreadNum, headle_file_list)  # 创建10线程
                # f 当前循环次数
                # 待处理的文件列表
                ...
                f += 1
            if len(section_data):
                MyMultithreadingStatistics.create_Thread(FileNum // 10, section_data)
        else:
            # print "section"+str(section_data)
            MyMultithreadingStatistics.create_Thread(FileNum, section_data)  # 创建线程
...
if __name__ == "__main__":
    try:
        #file_name=sys.argv[1]
        DataStatistics('D:\pythonobject\DNSdomaincheck\jiangsudns20170731')
    except:
        print "错误：请输入需要分析的文件名！"