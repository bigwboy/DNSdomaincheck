...
from  DataStatistics import StatisticsMain
...
if __name__ == "__main__":
    DataStatistics= StatisticsMain.DataStatistics('..\\AnalyzeData\\quanzhou.txt')
    if not DataStatistics :
        print '文件汇总统计错误~！'
    DomainList=RoadWriteFile.ReadFile('..\\OutFile\\Statistics.txt')
    CDNList=RoadWriteFile.ReadFile('.\\stencil\\CDN.xls')
    IPList=RoadWriteFile.ReadFile('.\\stencil\\IPlist.xls')
    check = CheckDomain(DomainList.ReturnData(),CDNList.ReturnData(),IPList.ReturnData())
    f = open('..\\OutFile\\CheckOverData.txt', 'w')
    for line in check:
        f.writelines(line+'\n')
    pass